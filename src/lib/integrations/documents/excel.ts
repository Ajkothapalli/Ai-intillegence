...
export function parseExcelBuffer(buffer: Buffer): ParsedDocument {
  const workbook = XLSX.read(buffer, { type: 'buffer' })
  const sheetName = workbook.SheetNames[0]
  if (!sheetName) return { type: 'research_context', content: '', source: 'excel' }

  const sheet = workbook.Sheets[sheetName]
  const rows = XLSX.utils.sheet_to_json<Record<string, unknown>>(sheet, { defval: '' })

  if (rows.length === 0) return { type: 'research_context', content: '', source: 'excel' }
